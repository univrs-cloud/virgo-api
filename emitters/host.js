...
const upgrade = (socket) => {
	...
	exec(`systemd-run --unit=upgrade-system --description="System upgrade" --wait --collect --setenv=DEBIAN_FRONTEND=noninteractive bash -c "echo $$ > ${upgradePidFile}; apt-get full-upgrade --with-new-pkgs -o Dpkg::Options::='--force-confold' -y -q > /var/www/virgo-api/upgrade.log 2>&1"`)
		.then(() => {
			checkUpgrade(socket);
		})
		.catch((error) => {
